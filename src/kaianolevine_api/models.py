...
class PipelineEvaluation(Base):
    ...
    __table_args__ = (
        # PIPE-002. One finding lands once per run, however many times it
        # is offered — a redelivered queue message, a retried POST from the
        # release workflow, a rerun of a job that already finished.
        #
        # Partial because run_id is nullable: a row with no run belongs to
        # no run and has nothing to be idempotent with respect to, and
        # making those unique on (repo, fingerprint) would reject a
        # repository legitimately reporting the same thing twice months
        # apart.
        Index(
            "ux_pipeline_evaluations_run_repo_fingerprint",
            "run_id",
            "repo",
            "fingerprint",
            unique=True,
    ...
    #: Hex SHA-256 over (violation_id, dimension, severity, finding,
    #: suggestion) — see services/evaluation_fingerprint.py, which is the
    #: only place it is computed, and migration 030, which backfilled the
    #: rows that predate it with the same digest.
    #:
    #: Written by the API rather than generated by Postgres: `convert_to`
    #: is STABLE, so the natural sha256 expression is rejected in a
